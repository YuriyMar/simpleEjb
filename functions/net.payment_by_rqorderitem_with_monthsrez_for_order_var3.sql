CREATE OR REPLACE FUNCTION net.payment_by_rqorderitem_with_monthsrez_for_order_var3 (
  p_startdate varchar,
  p_budgcode varchar,
...
--  and db.code = ANY ( p_budgcode )
  ...
  /*коды ддс */
  And ri.ddscodescode::text = any(string_to_array(p_strdds, ','))
...
--  and oi.budgetrefcode = ANY ( p_budgcode ) 
  ...
  /*коды ддс */
  And oi.ddscodescode::text = any(string_to_array(p_strdds, ','))
 ...
 loop 
      -- фактические оплаты в предыдущем периоде выберем по строке заявки
       for fact_pay in
        Select rqpaydocitem2billitem.summagen  /*пропорционально вычисляем оплаты строки заявки*/
          / coalesce((select sum(bi.countfact) from rqbillitem2orderitem oi , rqbillitem bi
