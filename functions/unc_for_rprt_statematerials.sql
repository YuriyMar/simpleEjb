...
RETURNS varchar AS
$body$
/* - 2011_10_27 - добавили статус на естимейте (4) что бы выбирался перенесеный материал на плане
и на факте просто покажем статус что он перенесен
*/
declare
jj record;
countsel1 numeric;
...
and rqfkor.kindcode = 1 /*Приходный ордер*/
and rqfkorstatu.code = rqfkor.statuscode
and rqfkorstatu.code in (1,2,4) /*для приходов складеных или счетчиков проведенных как ОЗ */
and fi2ei.countgen <> 0 /* не учитываем нулевые приходы так как по нему уже было дальше движение */
/* 22.04.2014 не учитываем приходы если по естимейту был документ по перемещению раньше */
and not exists ( select * from rqfkorderitem2enstmttm fi2ei2 , rqfkorderitem fi2 , rqfkorder f2
                      where fi2ei2.estimateitemcode =  selsprizn.eicode
...
case
when kindcode = 2 and rqfkorstatu.code = 1 /*Черновий*/
...
and rqfkit.code = fi2ei.fkorderitemrefcode
and rqfkor.code = rqfkit.fkorderrefcode
and rqfkor.kindcode /*= 2 */ in (2,4,5,6,13 , 28 , 29) /*Видатковий ордер или перемещение подрядчику*/
...
and rqfkit.code = fi2ei.fkorderitemrefcode
and rqfkor.code = rqfkit.fkorderrefcode
and rqfkor.kindcode in (2,5,6,13 , 28 , 29) /*Видатковий ордер */
...
from
enestimateitem2nstmttm enit2enit2 /*наличие материала на факте*/
inner join
(Select distinct estimateitemoutrefcode from enestimateitem2nstmttm /*наличие материала на плане*/
...
/*enplanwork enp ,*/
inner join enact2enplanwork act2wor on act2wor.plancode = ene.planrefcode /* стало */
...
Where
scc.statusrefcode = 2 /*в Акті (ОЗ-1 і інш.)*/
...
enit.code as eicode ,
case when enit.statusrefcode = 3 then 'У наявності'
when enit.statusrefcode = 4 then 'Перенесений'
...
enpl.staterefcode ,
enpl.elementrefcode ,
( select eld.ename ||' Инв.№'|| coalesce(eld.invnumber,'') || ' / (' ||eplstate.shortname || ')' ) as object
...
and enit.kindrefcode in ( 1 , 6 ) /*материалы и услуги */
and enit.statusrefcode in (1,2,3,4) /*добавили статус что бы выбирался перенесеный*/
...
-- and ( 0 = plancode ) /*!!!заменить формирование фильтра с клиента если выбираем конкретный план или не выбираем план */
...
-- 2013.03.04    and enpl.typerefcode <> 11 -- планы перевезення не берем
...
Where
(
( komplekt = 0 ) /*все материалы */
or
( ( komplekt = 1 ) and (countfact > aaaa) ) /*недоукомплектованые*/
or
( ( komplekt = 2 ) and (countfact < aaaa) ) /*переукомплектованые */
or
( ( komplekt = 3 ) and (countfact = aaaa) ) /*укомплектованы нормативн = переданым*/
)
-- order by departmentname , budgetname , object , workname , materialname
;

-- цикл по estimate
--  после того как залили данные в data_for_report_statematerials_sel1 делаем индексы
...
/* номер документа по kind (1 - прих ордер , 2- расходный ордер , 10 - акт ) */
...
/* для расходного ордера */
case
when kindcode in (2,4,5,6,13 , 28 , 29) and rqfkorstatu.code = 1 /*черновой*/
then rqfkor.molincode||' '||rqfkor.molinname
...
/*ищем приходы если счетчики */
UNION ALL
Select fi2ei.modify_time , fi2ei.estimateitemcode ,
fi2ei.countgen as countfact ,
rqfkor.moloutcode ,
rqfkor.moloutname , rqfkit.nomenclaturename || ' інв.№ ' || coalesce(scc.invnumber,'') as nomenclaturename , rqfkit.nomenclatureunitname
, 0 as partycode , rqfkor.kindcode ,substr(rqfkorstatu.name,1,10) as statusname ,
...
/*and ene.planrefcode = enp.code
and act2wor.plancode = enp.code было */
and ene.planrefcode = act2wor.plancode /* стало */
and act2wor.actrefcode = act.code
and fin.statusrefcode = 1 /*действительный*/
/*and act.statusrefcode = 3 проведенный в финколекции */
and actstatu.code = act.statusrefcode
...
and enit2enit1.typerefcode = 1 )
and enit2enit2.typerefcode = 1
and (SELECT jj.kindcode ) = 2 /*если признак месячній план */
...
and fin.estimateitemrefcode = ene.code
/*and ene.planrefcode = enp.code
and act2wor.plancode = enp.code было так */
and ene.planrefcode = act2wor.plancode /* стало так */
and act2wor.actrefcode = act.code
and fin.statusrefcode = 1 /*действительный*/
/* and act.statusrefcode = 3 проведенный в финколекции*/
and actstatu.code = act.statusrefcode
...
and enit2enit2.typerefcode = 1
and (SELECT jj.kindcode ) = 3 /*если признак задание план */
...
and (SELECT jj.kindcode ) = 4 /*если признак задание факт */
...
scoz.numberdoc as moloutcode , to_char(scut.dategen ,'dd.mm.yyyy') as moloutname ,
scc.name || ' інв.№ ' || coalesce(scc.invnumber,'') as nomenclaturename , 'ШТ' as nomenclatureunitname ,
0 as partycode , 10 as kindcode , scuts.name as statusname ,
case when act.statusrefcode = 3 then /* не показываем фактическое состояние материала если акт проведен */
' ' else coalesce(scc.molcode,'Лічільник абонента ') end as factstat
...
numberdoc  , datedoc ,movedateplan
       From (
       /*резервы под акты ищем с фактов */
       ...
       and f.estimateitemrefcode in (
               ...
               /*из месяного плана по задания факт*/
               Select en2en.estimateitemoutrefcode as estimatecode
                from enestimateitem2nstmttm en2en
                where en2en.estimateiteminrefcode in ( Select en2en.estimateitemoutrefcode as estimatecode
                from enestimateitem2nstmttm en2en
               ...
               -- and (SELECT jj.kindcode ) = 2
                and en2en.typerefcode = 1
                UNION
                /*из задания план до задания факт*/
                Select en2en.estimateitemoutrefcode as estimatecode
                from enestimateitem2nstmttm en2en
                where en2en.estimateiteminrefcode = jj.eicode
               -- and (SELECT jj.kindcode ) = 3
                and en2en.typerefcode = 1
                UNION
                /*если естимейт с факта */
                Select jj.eicode
               -- Where (SELECT jj.kindcode ) = 4
                )
       UNION
       /*факта еще может не быть поэтому приклеим резервы на плане у которого нет факта */
       select cast( jj.eicode  as varchar) as slujfield ,
        f.estimateitemrefcode as estimateitemcode , f.quantity as countfact ,
        f.mat_name as nomenclaturename , f.mu_name as nomenclatureunitname ,
        coalesce(f.party_id,0) as partycode , 0 as kindcode , ' ' as statusname , 0 as modify_time ,
...
' ' as moloutcode , 'Встановлено' as moloutname , 0 as prizn , ' ' as objfactname

, case when scc.molcode is null then 'Лічільник абонента ' else fmd.finmolcode|| ' ' || fmd.finmolname end as factstatt
, cast('' as varchar) as numberdoc
, cast('' as varchar) as datedoc
,( select min(em1.monthgen)|| ' ' || min(em1.yeargen) from enplanworkmovehistory em1
where em1.modify_time = (
...
--  после того как залили данные в data_for_report_statematerials_sel2 делаем индексы
